							...
							<tbody>
								...
								foreach($res as $row) { 
									...
									<tr <?php if(!empty($row['featureimg'])) { ?> style="background-color: #ffa099;" <?php } ?>>
										...
										<td><?=$row['grandtot']?></td>
										<td>
											<a href="<?=base_url()?>admin/Order/invoiceprint/<?=$row['orderno']?>" target="_blank">
												<button class="bnt btn-primary"><i class="fas fa-print"></i></button>
											</a>
										</td>
